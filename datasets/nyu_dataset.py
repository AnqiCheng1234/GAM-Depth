...
class NYUDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # load seg
        if do_flip:
            seg = self.seg_path + '/flip_nyu_seg_%d.npy' % (index)
        else:
            seg = self.seg_path + '/nyu_seg_%d.npy' % (index)
        inputs[('seg', 0, 0)] = np.load(seg)  # original: (288, 384), each element represents class label: 0~39

        magnitude = canny_edge_detector(np.array(self.resize[0](inputs[("color", 0, -1)])))
        ...
        inputs['svo_map_noise'] = torch.from_numpy(svo_map_resized,).float()
        # keypoints = np.concatenate((key_points, noise_points), axis=0)
        # inputs['dso_points'] = torch.from_numpy(keypoints,).float()
