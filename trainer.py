...
class Trainer:
    def __init__(self, options):
        ...
        # residual pose decoder
        self.models["pose_1"] = networks.PoseDecoder(
            self.models["pose_encoder"].num_ch_enc,
            num_input_features=1,
            num_frames_to_predict_for=2)
        self.models["pose_1"].to(self.device)
        self.parameters_to_train += list(self.models["pose_1"].parameters())

        # segmentation decoder
        if self.opt.using_seg:
            self.seg_loss_flag = True  # if true, add seg_loss into final loss
            self.models["seg"] = networks.SegDecoder(
                self.models["encoder"].num_ch_enc, self.opt.scales)
            self.models["seg"].to(self.device)
            self.parameters_to_train += list(self.models["seg"].parameters())
        ...
        if self.opt.train_dataset == "nyu":
            ...
            train_dataset = self.dataset(
                    ...
                    vps_path=self.opt.vps_path, seg_path=self.opt.seg_path,
                    return_vps=True)
        ...
        print("Type of photometric loss calculation:  ", self.opt.using_GAM)
        print("Using semantic constraints:  ", self.opt.using_seg)
        print("vps_path: ", self.opt.vps_path)
        print("seg_path: ", self.opt.seg_path)
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.start_epoch, self.opt.num_epochs):
            if self.opt.using_seg:
                if self.epoch >= 15:
                    self.seg_loss_flag = False
                print("using seg loss:", self.seg_loss_flag)
    ...
    def process_batch(self, inputs):
        ...
        outputs = {}
        # get depth and segmentation labels
        for i in [0]:
            ...
            if self.opt.using_seg:
                output_seg = self.models["seg"](features)
                output_seg = {(seg, i, scale): output_seg[(seg, scale)] for (seg, scale) in output_seg.keys()}
                outputs.update(output_seg)
        # get pose
        outputs.update(self.predict_poses(inputs, features))
        # get warped image based on Monodepth2
        self.generate_images_pred(inputs, outputs)
        # get residual pose
        outputs.update(self.predict_poses_1(inputs, outputs))
        # get final outputs from residual pose
        self.generate_images_pred_1(inputs, outputs)
    ...
    def predict_poses_1(self, inputs, outputs):
        """Predict poses between input frames for monocular sequences.
        """
        pose_feats = {f_i: outputs[("color", f_i, 0)] for f_i in self.opt.frame_ids_to_train[1:]}
        for f_i in self.opt.frame_ids_to_train[1:]:
            # To maintain ordering we always pass frames in temporal order
            if f_i < 0:
                pose_inputs = [pose_feats[f_i], inputs["color_aug", 0, 0]]
            else:
                pose_inputs = [inputs["color_aug", 0, 0], pose_feats[f_i]]
            pose_inputs = [self.models["pose_encoder"](torch.cat(pose_inputs, 1))]
            axisangle, translation = self.models["pose_1"](pose_inputs)
            # Invert the matrix if the frame id is negative
            outputs[("cam_T_cam_1", 0, f_i)] = transformation_from_parameters(axisangle[:, 0], translation[:, 0], invert=(f_i < 0))
            # dot product 2 pose from 2 pose net to get final pose
            outputs[("cam_T_cam_final", 0, f_i)] = outputs[("cam_T_cam", 0, f_i)] @ outputs[("cam_T_cam_1", 0, f_i)]
            del outputs[("cam_T_cam", 0, f_i)]
            del outputs[("cam_T_cam_1", 0, f_i)]
        return outputs
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids_to_train[1:]):
                T = outputs[("cam_T_cam", 0, frame_id)]
                cam_points = self.backproject_depth[source_scale](depth, inputs[("inv_K", source_scale)])
                pix_coords = self.project_3d[source_scale](cam_points, inputs[("K", source_scale)], T)
                outputs[("color", frame_id, scale)] = F.grid_sample(inputs[("color", frame_id, source_scale)],pix_coords,padding_mode="border")
    ...
    def generate_images_pred_1(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            outputs[("depth", 0, scale)] = depth
            for i, frame_id in enumerate(self.opt.frame_ids_to_train[1:]):
                T = outputs[("cam_T_cam_final", 0, frame_id)]
                cam_points = self.backproject_depth[source_scale](depth, inputs[("inv_K", source_scale)])
                outputs[('cam_points', source_scale)] = cam_points
                pix_coords = self.project_3d[source_scale](cam_points, inputs[("K", source_scale)], T)
                outputs[("color_final", frame_id, scale)] = F.grid_sample(inputs[("color", frame_id, source_scale)],pix_coords,padding_mode="border")
    ...
    def compute_reprojection_loss(self, pred, target):
        ...
        ssim_loss = self.ssim(pred, target).mean(1, True)  #torch.Size([4, 1, 192, 640])
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            # reprojection loss from MonoDepth2
            reprojection_losses = []
            identity_reprojection_losses = []
            for frame_id in self.opt.frame_ids_to_train[1:]:
                pred = outputs[("color_final", frame_id, scale)]
            ...
            combined_new = reprojection_loss
            to_optimise, idxs = torch.min(combined, dim=1)
            _, idxs_new = torch.min(combined_new, dim=1)
            outputs["identity_selection/{}".format(scale)] = (idxs > identity_reprojection_loss.shape[1] - 1).float()
            outputs["idxs/-2/{}".format(scale)] = (idxs_new == 0).float()
            outputs["idxs/-1/{}".format(scale)] = (idxs_new == 1).float()
            outputs["idxs/1/{}".format(scale)] = (idxs_new == 2).float()
            outputs["idxs/2/{}".format(scale)] = (idxs_new == 3).float()
            outputs["fused_min/{}".format(scale)] = torch.zeros(self.opt.batch_size, 3, self.opt.height, self.opt.width)
            for i in range(3):
                outputs["fused_min/{}".format(scale)][:, i] = \
                    outputs[("color_final", -2, scale)][:, i] * outputs["idxs/-2/{}".format(scale)] + \
                    outputs[("color_final", -1, scale)][:, i] * outputs["idxs/-1/{}".format(scale)] + \
                    outputs[("color_final", 1, scale)][:, i] * outputs["idxs/1/{}".format(scale)] + \
                    outputs[("color_final", 2, scale)][:, i] * outputs["idxs/2/{}".format(scale)]

            if self.opt.using_GAM == "GAM":
                magnitude_mask = self.opt.beta_GAM + (1 - self.opt.beta_GAM) / (1 + torch.exp(self.opt.gamma1_GAM * (- inputs[("magnitude")]) + self.opt.gamma2_GAM))
                to_optimise_svo = to_optimise * 10 * magnitude_mask
                loss += to_optimise_svo.mean()
            elif self.opt.using_GAM == "keypoints":
                to_optimise_svo = to_optimise * inputs['svo_map']
                loss += to_optimise_svo.sum() / inputs['svo_map'].sum()
            elif self.opt.using_GAM == "averaging":
                to_optimise_svo = to_optimise
                loss += to_optimise_svo.mean()

            if self.opt.using_seg:
                # segmentation cross entropy loss
                seg_proxy = inputs[('seg', 0, 0)].to(torch.long)
                seg_pred = outputs[('seg_pred', 0, 0)]
                outputs[('seg_pred_label', 0, 0)] = torch.argmax(seg_pred, dim=1)
                seg_loss_funtion = nn.CrossEntropyLoss(ignore_index=0)
                seg_loss = seg_loss_funtion(seg_pred, seg_proxy)
                if self.seg_loss_flag:
                    loss += self.opt.lambda_seg * seg_loss
            ...
            loss += self.opt.lamda_disparity_smoothness * smooth_loss / (2 ** scale)
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(1, self.opt.batch_size)):  # write a maxmimum of four images
            ...
            '''
            writer.add_image(
                "seg_proxy_{}/{}".format(0, j),
                inputs['seg', 0, 0][j].unsqueeze(0).data * 255 / 39, self.step)
            # writer.add_image(
            #     "seg_pred_{}/{}".format(0, j),
            #     outputs[('seg_pred_label', 0, 0)][j].unsqueeze(0).data * 255 / 39, self.step)
            writer.add_image(
                "grad_magnitude",
                normalize_image(inputs[("magnitude")][j]), self.step)
    ...
    def compute_smooth_norm(self, inputs, outputs):
        ...
        for scale in range(self.num_scales):
            ...
            mmap, mmap_mask, mmap_mask_thresh = compute_mmap(self.opt.batch_size, pred_norm, vps, self.opt.height,
                                                             self.opt.width, self.epoch, self.opt.d2n_nei)
    ...
    def compute_seg(self, inputs, outputs):
        ...
        for scale in range(self.num_scales):
            ...
            rgb_down = self.pdist(rgb[:, :, 1:].permute(0, 2, 3, 1),
                                  rgb[:, :, :-1].permute(0, 2, 3, 1))  # pairwise distance, euclidean distance
            rgb_right = self.pdist(rgb[:, :, :, 1:].permute(0, 2, 3, 1), rgb[:, :, :, :-1].permute(0, 2, 3, 1))
            ...
            norm_down = self.pdist(aligned_norm[:, :, 1:].permute(0, 2, 3, 1), aligned_norm[:, :, :-1].permute(0, 2, 3,
                                                                                                               1))  # "permute" is needed because pytorch upgrade to 1.10.1
            norm_right = self.pdist(aligned_norm[:, :, :, 1:].permute(0, 2, 3, 1),
                                    aligned_norm[:, :, :, :-1].permute(0, 2, 3, 1))
